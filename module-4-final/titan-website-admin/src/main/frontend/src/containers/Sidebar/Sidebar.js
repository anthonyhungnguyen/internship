...
import {
  Badge, Nav, NavItem, NavLink as RsNavLink,
} from 'reactstrap';
...
class Sidebar extends Component {
  ...
  render() {
    ...
    const navList = (items, items_roles) => {
      ...
      const filtered = items.filter(item => (this.isValidRole(this.props.roles, item.roles))).map((item, index) => navType(item, index, items_roles));
      return filtered
    };
