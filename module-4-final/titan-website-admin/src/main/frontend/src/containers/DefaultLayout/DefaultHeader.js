	...
	({ history }) =>
		fakeAuth.isAuthenticated() ? (
			<div>
				...
				>
					<i className="icon-logout fa-lg" /> &nbsp; Sign out
				</Button>
			</div>
		) : (
			<Button
			...
			>
				{' '}
				<i className="icon-logout fa-lg" />&nbsp; You are not logged in
			</Button>
		)
)
		...
		return (
            ...
            <NavLink to="/dashboard" className="nav-link" >Dashboard</NavLink>
					...
					<NavItem className="px-3">
						<UncontrolledDropdown nav direction="down">
							...
							<DropdownMenu right>
								...
								<DropdownItem
									onClick={() => {
										this.changeFont('fontNunito')
									}}
								>
									<i className="fa fa-font" /> Nunito
								</DropdownItem>
              ...
              <DropdownItem><i className="fa fa-shield"></i> Lock Account</DropdownItem> */}
								<DropdownItem
									onClick={() => {
										this.changeFont('fontCabinCondensed')
									}}
								>
									<i className="fa fa-font" /> Cabin Condensed
								</DropdownItem>
							</DropdownMenu>
						</UncontrolledDropdown>
					</NavItem>
					...
					<NavItem className="d-md-down-none px-3">
						<NavLink to="#" className="nav-link">
							{localStorage.getItem('roles') ? (
								this.getTag(JSON.parse(localStorage.getItem('roles'))[0])
							) : (
								<span />
							)}
						</NavLink>
					...
					<UncontrolledDropdown nav direction="down">
						<DropdownToggle nav>
							...
							<Button color="danger" size="sm" outline>
								{' '}
								<i className="fa fa-user fa-lg" /> &nbsp; {fakeAuth.getUsername()}
							</Button>
						</DropdownToggle>
						{/* <DropdownMenu right>
