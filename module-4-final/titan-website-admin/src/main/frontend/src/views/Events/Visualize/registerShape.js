...
G6.registerNode(
    'flow-rect', {
        ...
        draw: (cfg, group) => {
            ...
            // 逻辑不应该在这里判断
            const rectConfig = {
                width: !label.includes('Code')?210:300,
                height: (!label.includes('Code')?84:94)+(conditions?conditions.length*10:0) +(decisionType?1:0),
                lineWidth: 1,
                fontSize: 12,
                fill: '#fff',
                radius: 4,
            ...
            group.addShape('text', {
                attrs: {
                    ...
                    x: 12,
                    y: 30,
                    text: !label.includes('Code')?name.length>25?name.substr(0,25)+'...':name:name,
                    fontSize: !label.includes('Code')?14:12,
                    fontWeight:'bold',
                    fill: '#000',
                    cursor: 'pointer',
                    textAlign:'left',
