...
import {getActionCodes} from '../../api/action-code'
...
class NewForm extends React.Component {
    constructor(props){
      ...
      this.state = {
        ...
        actionCode:props.ruleData?props.ruleData.actionCode.id:1,
    ...
    componentWillMount=async()=>{
        ...
        const actionCodeAPI = await getActionCodes(fakeAuth.getAccessToken(),0,1000);
        ...
        const actionCodes = actionCodeAPI.data.list;
              ...
              return (
                ...
                {Object.keys(ele).length>0?Object.keys(ele).map(field=>{
                    if (field === 'infoCode'){
                      ...
                      return (<td scope='row'>
                            [{ele[field].id}] {ele[field].message}
                        </td>)
                    }
            ...
            <FormGroup row>
                  ...
                  <FormGroup row>
                    ...
                    <Select
                        ...
                        showSearch
                        onChange={this.handleChangeActionCode}
                      ...
                      >
                      {this.state.actionCodeSuggest.map(ele=>(
                        <Option value={ele.id}>[{ele.id}] {ele.message}</Option>
                      ))}
                    </Select>
                    </Col>
                  </FormGroup>
