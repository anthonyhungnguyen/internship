...
export default (() => {
    ...
    useEffect(() => {
        const fetchRecentTransaction = async () => {
            const response = await fetch('http://localhost:8085/api/user_device/test/', {
                ...
                body: JSON.stringify({
                    query: `FOR v, e IN 1..1 ANY @id @@col FILTER e.type == @type SORT DATE_TIMESTAMP(DATE_ISO8601(e.reqDate)) DESC LIMIT @limit RETURN e`,
                    bindVars: {
                        '@col': 'users_devices',
                        id: `devices/${deviceId}`,
                        type: 'transaction',
    ...
    return <Card title="5 Recent Transaction Activities" headStyle={{ fontWeight: 'bold', fontSize: '1.3em' }} hoverable={true}>
        {recentTransaction && <Descriptions column={1} bordered>
            <Descriptions.Item label="From Last Payment">
                {recentTransaction['lastTransFromNow']}
            </Descriptions.Item>
            <Descriptions.Item label="App Types"><ul>
                {Object.keys(recentTransaction['appTypes']).map(app => <li>{app} - {recentTransaction['appTypes'][app].toLocaleString('en-EN', { style: 'currency', currency: 'VND' })}</li>)}
