...
import {getActionCodes,editActionCode,createActionCode,deleteActionCode} from '../api/action-code'
...
class UserRow extends React.Component {
    ...
    handleDeleteInfoCode=async()=>{
      try{
        await deleteActionCode(this.props.user.id,{},fakeAuth.getAccessToken())
        swal("Thông báo!", "Delete action code thành công!", "success").then(res=>{
          if (res){
        ...
        swal("Thông báo!", "Delete action code lỗi! "+e, "error")
      }
    }
    ...
    handleEditInfoCode=async()=>{
      ...
      try{
        ...
        await editActionCode(this.props.user.id,newInfoCode,fakeAuth.getAccessToken())
        swal("Thông báo!", "Edit action code thành công!", "success").then(res=>{
          if (res){
            window.location.reload(false)
          }
        })
        ...
        swal("Thông báo!", "Edit action code lỗi! "+e, "error")
      }
    }
  ...
  render(){
    ...
    const infoCodeLink = `/action-code/${user.id}`
        ...
        <td><Link to={infoCodeLink}><strong>{user.id}</strong></Link></td>
          ...
          <Badge color={'primary'} onClick={this.toggleEditInfoCode}>
            ...
            <Modal size="md" isOpen={this.state.modalEditInfoCode} toggle={this.toggleEditInfoCode}>
                    ...
                    Edit actionCode {this.props.user.id}
                    </ModalHeader>
          ...
          <Badge color={'danger'} onClick={this.toggleDelete}>
            ...
            <Modal isOpen={this.state.modalDelete} toggle={this.toggleDelete}>
              <ModalHeader>
                Delete actionCode: {this.props.user.id}
              </ModalHeader>
              <ModalBody>
...
export default class App extends React.Component {
  ...
  handleNewInfoCode=async()=>{
    ...
    try{
      let newInfoCode = this.state.newInfoCode
      newInfoCode.createAt = new Date().getTime()
      await createActionCode(newInfoCode,fakeAuth.getAccessToken())
      swal("Thông báo!", "Tạo action code thành công!", "success").then(res=>{
        if (res){
          window.location.reload(false)
        }
      })
      ...
      swal("Thông báo!", "Tạo action code lỗi! "+e, "error")
    }
  }

  componentWillMount = () => {
    getActionCodes(fakeAuth.getAccessToken(), 0, 10).then(res => {
  ...
  onChangePage=(page,pageSize)=>{
      ...
      getActionCodes(fakeAuth.getAccessToken(), page-1, pageSize).then(res => {
        console.log(res.data.list)
        this.setState({
            data: res.data.list,
            masterData: res.data.list,
  ...
  onShowSizeChange=(page,pageSize)=>{
    getActionCodes(fakeAuth.getAccessToken(), 0, pageSize).then(res => {
      console.log(res.data.list)
      this.setState({
          data: res.data.list,
          masterData: res.data.list,
